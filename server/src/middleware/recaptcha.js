...
export async function requireRecaptcha(req, _res, next) {
  try {
    ...
    if (!result.ok) {
      // Failed reCAPTCHA is a strong bot signal — feed the firewall.
      recordSuspicious(req, 'recaptcha_failed', { path: req.originalUrl, userAgent: req.get('user-agent') });
      return next(forbidden('reCAPTCHA verification failed', 'recaptcha'));
    }
    next();
  } catch (err) {
    next(err);
